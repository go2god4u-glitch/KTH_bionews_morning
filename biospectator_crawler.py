...
■ 수정 이력
  ...
  ─────────────────────────────────────────────────────────────────────────────
  [추가 내용]
  1. 더바이오 검색 함수 (thebio_search)
     - POST 방식 검색 (sc_word 파라미터), 로그인 불필요
     - 날짜 형식 YYYY.MM.DD → YYYY-MM-DD 정규화
     - 검색 결과: <li> → <H2 class="titles"> 구조 파싱

  2. 더바이오 기사 크롤링 함수 (thebio_crawl_article)
     - 제목: <h1 class="heading">
     - 날짜: <ul class="infomation"> 안의 '입력 YYYY.MM.DD HH:MM'
     - 본문: <article id="article-view-content-div">
     - 사진: figure.photo-layout 보존 (img src/alt만 유지, 나머지 속성 제거)

  3. 공통 유틸 함수 분리
     - build_search_variants(): 키워드 → 대/소문자 + 별칭 변형 목록 생성
     - highlight_keywords(): 본문 키워드 형광 강조 (기존 로직 분리)

  4. HTML 리포트 (GitHub Pages) 개선
     - 두 사이트 기사 모두 표시
     - 출처 뱃지: BioSpectator(파란색) / 더바이오(초록색)
     - 상단바: 전체 건수 + BioSpectator/더바이오 건수 별도 표시

  5. 이메일 방식 유지 + 개선
     - 이메일 본문: 기존과 동일 (BioSpectator 기사 미리보기)
     - 더바이오 기사가 있으면 상단에 초록 배너로 건수 안내
     - GitHub Pages 링크 클릭 시 두 사이트 기사 모두 확인 가능
     - build_email_body_html(): 이메일용 인라인 스타일 HTML 별도 생성

  [테스트 결과]
  - BioSpectator 로그인 성공 확인
  - 더바이오 GLP-1 키워드 기사 2건 크롤링 성공
  - 이미지 정상 출력 확인
  - 이메일 발송 성공 (e2102208@donga.co.kr)
  ─────────────────────────────────────────────────────────────────────────────
